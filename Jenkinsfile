	...
	stage("Release scope") {
			    ...
			    if (env.RELEASE_SCOPE == "AWSCodeDeploy")
		    {
			   ...
			   // build job: 'AWSCodeDeploy'
			    ...
			    step([$class: 'AWSCodeDeployPublisher', applicationName: 'CodeDeploy', awsAccessKey: '$AWS_ACCESS_KEY_ID', awsSecretKey: '$AWS_SECRET_ACCESS_KEY', deploymentGroupAppspec: false, deploymentGroupName: 'codedeploygroup', deploymentMethod: 'CodeDeployDefault.AllAtOnce', includes: '**', proxyPort: 0, region: 'us-east-1', s3bucket: 'aws-code-deploy-test-jenkins'])
			    }
